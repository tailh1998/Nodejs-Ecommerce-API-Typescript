...
const getDiscountByCode = async (code: string) => await DiscountModel.findOne({ code }).lean()
...
const updateDiscount = async (filter: Object, payload: Object) => {
  return await DiscountModel.findOneAndUpdate(filter, payload, { new: true }).lean()
}
